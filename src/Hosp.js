...
htmlToImage.toPng(node)
  ...
  return (
    <div className="back">
      ...
      <h2 className="dist">{t("Nearly")} {parseFloat(d).toFixed(2)} {t("Kms away")}</h2>
      </div>
      </div>
      ...
      <div className="charge">
        <h3 className="design">{props.type==="government"?t("Government"):t("Private")}</h3>
        <h3 className="design1">{props.type==="government"?t("Non Chargeable"):t("Chargeable")}</h3>
      </div>

      <div className="oxygen">
            <div>
                <h3 className="format">{t("With Oxygen")}</h3>
                <h3 className="format1"><span>{props.oxy}</span>/{props.twithoxy}</h3>
            </div>

            <div className="shift">
                <h3 className="format">{t("Without Oxygen")}</h3>
                <h3 className="format1"><span>{props.withoutoxy}</span>/{props.twithoutoxy}</h3>
            </div>
      ...
      <div>
                ...
                <h3 className="format1"><span>{props.icuventi}</span>/{props.ticuventi}</h3>
            </div>

            <div>
                <h3 className="format">{t("ICU Without Ventilators")}</h3>
                <h3 className="format1"><span>{props.icunonventi}</span>/{props.ticunonventi}</h3>
            </div>
        ...
        <div className="repshare">
          <div >
        <h7 className="reportss" onClick={()=>settrig(true)}>{t("Report Error")}</h7>
        <Popup trigger={trig} strig={settrig} hospitalname={props.hospitalname} address={props.address} phoneno={props.phoneno}>
        </Popup>
